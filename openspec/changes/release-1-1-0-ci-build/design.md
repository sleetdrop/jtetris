...
Use a separate manually-triggerable and tag-triggered workflow for release artifacts. The workflow builds artifacts from the checked-out commit and uploads them as workflow artifacts. It does not create, edit, or publish a GitHub Release.
...
Native app images remain platform-built, not cross-compiled. macOS artifacts are produced on macOS runners, and Windows artifacts are produced on Windows runners. The standalone jar remains the fallback for platforms without a native app image.
