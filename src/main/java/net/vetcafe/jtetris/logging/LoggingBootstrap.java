...
public final class LoggingBootstrap {
    ...
    public static synchronized void initialize() {
        ...
        LoggingSettings settings =
                prepareBundled(systemProperties, ApplicationDataPaths.currentRoot(), Files::createDirectories);
    ...
    public static long edtWatchdogThresholdMs() {
        LoggingSettings settings = currentSettings;
        return settings == null ? LoggingSettings.DEFAULT_WATCHDOG_THRESHOLD_MS : settings.edtWatchdogThresholdMs();
    }

    static LoggingSettings prepareBundled(
            Properties properties, Path applicationRoot, DirectoryCreator directoryCreator) {
        ...
        properties.setProperty(
                "jtetris.log.level.resolved", settings.globalLevel().name());
        properties.setProperty(
                "jtetris.log.input.level.resolved", settings.inputLevel().name());
        properties.setProperty(
                "jtetris.log.dir.resolved", settings.logDirectory().toString());
        properties.setProperty("jtetris.log.maxFileSize.resolved", settings.maxFileSize());
        properties.setProperty("jtetris.log.maxHistory.resolved", Integer.toString(settings.maxHistory()));
        ...
        try {
            ...
            System.err.println("JTetris logging: cannot initialize log directory "
    ...
    static void installUncaughtExceptionHandler() {
        ...
        Thread.setDefaultUncaughtExceptionHandler((thread, error) -> {
            LoggerFactory.getLogger(APP_LOGGER).error("event=uncaught_exception thread={}", thread.getName(), error);
            if (previous != null) {
